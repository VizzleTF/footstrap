...
7. **Не хардкодить пути.** Всегда `{{ media }}` и `{{ resource }}`, не
   ...
   basename (в footstrap так работает вторая раскладка: медиа-каталог
   `footstrap-top` — симлинк на `footstrap`, обе отдают один `cascade.css`).
...
13. **Минификатор luci.mk ломает современный CSS/JS.** По умолчанию сборка
    минифицирует cascade.css и menu-JS. Минификатор манглит `:has()`,
    `color-mix()`, вложенный `calc()`, современный ES → пакет ставится, но
    вёрстка «съезжает». Обязательно `LUCI_MINIFY_CSS:=0` + `LUCI_MINIFY_JS:=0`
    в Makefile (симптом: cascade.css в пакете сильно меньше исходного). См.
...
> **Как это выглядит в footstrap.** `cascade.css` не лежит в git — он собирается
> из `styles/` (`build-css.sh`, docs/17), `mobile.css` нет: адаптив внутри той же
> сборки (`styles/theme/90-responsive.css`). Шапка/подвал разбиты на
> `partials/*.ut`, общие для обеих раскладок. Клиентского меню три файла:
> `menu-footstrap.js`, `menu-footstrap-top.js` и общий `menu-footstrap-common.js`
> (+ `fs-select.js`). Своего `sysauth.ut` нет — логин рисует стоковый шаблон
> luci-base. Симлинк-варианты `-dark`/`-light` не используются: uci-записей две,
> по одной на раскладку, а режим/палитра — клиентские переключатели (док 04);
> `postrm` при этом всё равно удаляет и легаси-имена.

## Стратегия: старт от bootstrap
...
> **Где footstrap сейчас.** Пункты 1–3 пройдены, п.4 — снят: правила стоковой
> `cascade.css` живут в `styles/base/` (`@layer base`) и поглощаются по одному —
> блок удаляется, `cssdiff` по обеим раскладкам показывает, что тема потеряла, и
> это переписывается в свой компонент, пока диф не станет пустым (docs/17).
> Вместо диффа с upstream покрытие виджетов проверяется по `docs/gallery.html`
> (docs/12) — там отрисованы все классы, которые LuCI и сторонние `luci-app-*`
> вообще умеют выдавать. В рантайме тема зависит только от `luci-base`.
